...
# Coordenadas fijas de puntos específicos en Medellín
coordenadas_puntos = {
    "A": (6.2442, -75.5812),
    "B": (6.2444, -75.5714),
    "C": (6.2678, -75.5681),
...
def generar_mapa_waze(start, end, G):
    ...
    # Animación tipo Waze con AntPath configurado profesionalmente
    AntPath(
        locations=coords,
        dash_array=[20, 30],
        delay=300,
...
class GrafoAnimado:
    ...
    def animar(self, G, ruta_optima):
        ...
        def update(frame):
            ...
            nx.draw_networkx_edges(G, self.pos, edgelist=blocked_edges, width=4, edge_color="red", style="dotted", alpha=0.7,
                                   ax=self.ax)
...
class App:
    def __init__(self, root):
        ...
        ttk.Label(self.frame, text="Simulador de Rutas Inteligente", style="Title.TLabel").grid(row=0, column=0, columnspan=3,
                                                                                              pady=(0, 30))
    ...
    def toggle_arista(self, u, v):
        ...
        # Animación tipo Waze con AntPath
        AntPath(
            locations=coords,
            dash_array=[20, 30],
            delay=300,
